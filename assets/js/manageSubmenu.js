        ...
        $.ajax({
            ...
            success: (data) => {
                ...
                for (let i = 0; i < data.length; i++) {
                    ...
                    <td class="text-center">${data[i].ssm_name}</td>
                    <td class="text-center">${data[i].ssm_controller}</td>
                    <td class="text-center">${data[i].ssm_updated_date}</td>
                    <td class="text-center">${data[i].ssm_updated_by}</td>
                    <td class="text-center">
                      <button class="btnStatus btn badge bg-label-${data[i].ssm_status_flg == 1 ? 'success' : 'secondary'} me-1" id="flgStatus" data-sa-id="${data[i].ssm_id}" value="${data[i].ssm_status_flg}">
                        ${data[i].ssm_status_flg == 1 ? 'Enable' : 'Disable'}
                      </button>
                    </td>
                    <td class="text-center">

                    <button class="btn btn-label-danger tblEditBtn btn btn-icon" data-repeater-delete="" data-bs-toggle="modal" data-bs-target="#mdlEdit" id="btnEdit" data-id="${data[i].ssm_id}">
